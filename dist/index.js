...
var VirtualModule = class {
  ...
  get bindModule() {
    return import_Namespace.default.globals.get(this.getName());
  }
  get file() {
  ...
  createReferences(ctx, graph) {
    this.getReferences().forEach((local, classname) => {
      let module2 = import_Namespace.default.globals.get(classname);
      if (module2) {
        let dep = null;
...
function parseHttpAnnotation(ctx, stack) {
  ...
  const providerModule = moduleClass ? import_Namespace2.default.globals.get(moduleClass.value) : null;
  if (!providerModule) {
...
function parseImportDeclaration(ctx, stack, context = null, graph = null) {
  ...
  if (stack.source.isLiteral) {
    ...
    if (compilation && !compilation.isDescriptorDocument()) {
      ...
      if (!isExternalDependency(ctx.options.dependences.externals, source, context)) {
        specifiers = createFormatImportSpecifiers(stack);
    ...
    if (source) {
      if (specifiers.length > 0) {
        specifiers.forEach((spec) => {
          importSource = ctx.addImport(source, spec.local, spec.imported, spec.stack);
        });
      } else {
...
function createHttpAnnotationNode(ctx, stack) {
  ...
  const System = import_Namespace2.default.globals.get("System");
  const Http = import_Namespace2.default.globals.get("net.Http");
...
function createReadfileAnnotationNode(ctx, stack) {
  ...
  const make = (list) => {
    return list.map((object) => {
      ...
      if (object.isFile) {
        properties.push(ctx.createProperty("isFile", ctx.createLiteral(true)));
      }
      if (object.content) {
        properties.push(ctx.createProperty("content", ctx.createChunkExpression(object.content)));
      }
      if (object.children) {
        properties.push(ctx.createProperty("children", ctx.createArrayExpression(make(object.children))));
      }
      return ctx.createObjectExpression(properties);
    });
  };
    ...
    importSource.specifiers.forEach((spec) => {
      if (spec.type === "default" || spec.type === "namespace") {
        ...
        if (spec.type === "default") {
          ...
          const context = importSource.getSourceContext();
          const isModule2 = import_Utils.default.isModule(module2);
...
function createCJSExports(ctx, exportManage) {
...
function createESMExports(ctx, exportManage) {
  let importSpecifiers = /* @__PURE__ */ new Map();
  let exports2 = [];
  let imports = [];
  let declares = [];
...
function getMethodOrPropertyAlias(ctx, stack, name = null) {
  if (has(stack, "Common.getMethodOrPropertyAlias")) {
    return get(stack, "Common.getMethodOrPropertyAlias");
  }
  ...
  set(stack, "Common.getMethodOrPropertyAlias", resolevName);
  return resolevName;
}
function getMethodOrPropertyHook(ctx, stack) {
  if (!stack) return null;
  if (has(stack, "Common.getMethodOrPropertyHook")) {
    return get(stack, "Common.getMethodOrPropertyHook");
  }
  ...
  set(stack, "Common.getMethodOrPropertyHook", invoke);
  return invoke;
}
...
function createModuleReferenceNode(ctx, stack, className) {
  let gloablModule = import_Namespace2.default.globals.get(className);
  if (gloablModule) {
...
function AssignmentExpression_default(ctx, stack) {
  ...
  if (isMember) {
    if (stack.left.computed) {
      ...
      if (!hasDynamic && !import_Utils2.default.isLiteralObjectType(stack.left.object.type())) {
        isReflect = true;
      }
    } else if (!desc2 || desc2.isAnyType) {
      isReflect = !import_Utils2.default.isLiteralObjectType(stack.left.object.type());
    }
  }
  if (isReflect) {
...
function DeclaratorDeclaration_default(ctx, stack, type) {
}
...
function ForOfStatement_default(ctx, stack) {
  const type = import_Utils5.default.getOriginType(stack.right.type());
  if (import_Utils5.default.isLocalModule(type) || stack.right.type().isAnyType) {
...
function Identifier_default(ctx, stack) {
  ...
  if (desc2 !== stack.module && import_Utils6.default.isClassType(desc2)) {
...
function ImportDeclaration_default(ctx, stack) {
  parseImportDeclaration(ctx, stack);
  return null;
}
...
function JSXAttribute_default(ctx, stack) {
  ...
  if (ns === "@binding" && stack.value) {
    ...
    if (!has2 && stack.value.isJSXExpressionContainer) {
...
function addImportReference(ctx, desc2) {
  if (desc2 && desc2.isStack && desc2.imports) {
    const isDecl = desc2.isDeclaratorVariable || desc2.isDeclaratorFunction;
    if (isDecl && Array.isArray(desc2.imports)) {
      desc2.imports.forEach((item) => {
...
function MemberExpression(ctx, stack) {
  ...
  if (description && description.isModule && objectType && !objectType.isLiteralObjectType && import_Utils8.default.isTypeModule(description)) {
    ...
    if (import_Utils8.default.isTypeModule(objectDescriptor)) {
      ...
      addImportReference(ctx, objectDescriptor);
      addImportReference(ctx, description);
    }
  }
  if (!description || import_Utils8.default.isType(description) && description.isAnyType && !stack.optional) {
    let isReflect = true;
    if (description) {
      isReflect = false;
...
function MethodDefinition_default(ctx, stack, type) {
  ...
  node.modifier = import_Utils9.default.getModifierValue(stack);
  node.kind = "method";
...
function NewExpression_default(ctx, stack) {
  ...
  if (desc2 !== stack.module && import_Utils10.default.isTypeModule(desc2)) {
    ctx.addDepend(desc2, stack.module);
  }
  const node = ctx.createNode(stack);
  node.callee = ctx.createToken(stack.callee);
...
function PropertyDefinition_default(ctx, stack) {
  ...
  node.modifier = import_Utils11.default.getModifierValue(stack);
  node.static = !!stack.static;
...
function UnaryExpression_default(ctx, stack) {
  ...
  if (operator === "delete" && stack.argument.isMemberExpression) {
    ...
    if (desc2 && desc2.isAnyType) {
      ...
      if (!hasDynamic && !import_Utils12.default.isLiteralObjectType(stack.argument.object.type())) {
...
function UpdateExpression_default(ctx, stack) {
  ...
  if (isMember) {
    ...
    if (stack.argument.computed) {
      const hasDynamic = desc2 && desc2.isComputeType && desc2.isPropertyExists();
      if (!hasDynamic && !import_Utils13.default.isLiteralObjectType(stack.argument.object.type())) {
        isReflect = true;
      }
    } else if (desc2 && desc2.isAnyType) {
      isReflect = !import_Utils13.default.isLiteralObjectType(stack.argument.object.type());
    }
    if (isReflect) {
...
function createAsset(sourceFile, id = null, type = null) {
  if (!type) {
    type = import_path2.default.extname(sourceFile);
    if (type.startsWith(".")) {
  ...
  let asset = records4.get(key);
  if (!asset) {
    records4.set(sourceFile, asset = new Asset(sourceFile, type, id));
  }
  return asset;
}
...
var Context = class _Context extends import_Token.default {
  ...
  createModuleImportReferences(module2, graph = null) {
    ...
    module2.getStacks().forEach((stack) => {
  ...
  createAssets(context, graph) {
    ...
    if (assets && assets.size > 0) {
      assets.forEach((asset) => {
        if (asset.file) {
          if (!isExternalDependency(this.options.dependences.externals, asset.resolve, context)) {
            let asset2 = createAsset(asset2.resolve);
            asset2.local = asset2.assign;
            if (this.isLoadAssetsRawCode(asset2.stack, asset2.resolve)) {
              const { id, index, type: ext } = asset2;
              asset2.sourceId = this.getModuleResourceId(context, { id, index, ext, type: "embedAssets" });
            } else {
              asset2.sourceId = this.getModuleImportSource(asset2.resolve, context, asset2.file);
            }
            let importSource = this.addImport(asset2.sourceId, asset2.local);
            importSource.setSourceTarget(asset2);
            importSource.setSourceContext(context);
            if (graph) {
              graph.addAsset(asset2);
              graph.addImport(importSource);
            }
          }
        } else if (asset.type === "style" && context) {
          const { index, type, attrs = {} } = asset2;
          ...
          let asset2 = createStyleAsset(import_Utils16.default.normalizePath(context.file), index);
          let importSource = this.addImport(source);
          importSource.setSourceTarget(asset2);
          importSource.setSourceContext(context);
          asset2.sourceId = source;
          if (asset2.content) {
            asset2.code = asset2.content;
          }
          if (graph) {
            graph.addAsset(asset2);
            graph.addImport(importSource);
  ...
  getGlobalRefName(stack, name, group = null) {
    if (!stack) {
      stack = import_Utils16.default.isCompilation(this.target) ? this.target.stack : this;
    }
    if (group) {
  ...
  genUniFileName(source) {
    ...
    if (import_path3.default.isAbsolute(source) || source.includes("?")) {
      let ext = import_path3.default.extname(source);
      return import_path3.default.basename(source, ext) + "-" + (0, import_crypto2.createHash)("sha256").update(source).digest("hex").substring(0, 8) + ext;
    }
    return source;
  }
  getOutputDir() {
  ...
  getOutputRelativePath(source, context) {
    if (typeof source === "string" && isExternalDependency(this.options.dependences.externals, source, context)) {
      return source;
    }
  ...
  emit(buildGraph) {
    ...
    import_fs2.default.mkdirSync(import_path3.default.dirname(outfile), { recursive: true });
  ...
  error(message, stack = null) {
    if (this.target) {
      ...
      if (!range && import_Utils16.default.isStack(stack)) {
...
function createBuilder(plugin2) {
  const buildContext = (records6 = /* @__PURE__ */ new Map(), options = {}) => {
    const builder = async (compiOrVModule) => {
      ...
      let buildGraph = null;
      let ctx = new Context_default(plugin2, compiOrVModule);
...
var package_default = {
  name: "es-transform",
  version: "0.0.1",
  description: "Code Transform For EaseScript",
  main: "dist/index.js",
  scripts: {
...
function parsePolyfillModule(file, createVModule2) {
  let content = import_fs3.default.readFileSync(file).toString();
  let references = [];
  let namespace = "";
  let requires = [];
  let exportName = null;
...
function createPolyfillModule(dirname) {
  ...
  import_fs3.default.readdirSync(dirname).forEach((filename) => {
    const filepath2 = import_path4.default.join(dirname, filename);
    if (import_fs3.default.statSync(filepath2).isFile()) {
      parsePolyfillModule(filepath2, _createVModule);
    } else if (import_fs3.default.statSync(filepath2).isDirectory()) {
