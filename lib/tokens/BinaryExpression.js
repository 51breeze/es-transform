...
import { createStaticReferenceNode , gloablModules} from '../core/Common';
     ...
     if( operator ==="is" || operator==="instanceof" ){
          ...
          if(operator === "is"){
               ...
               if(Namespace.globals.get("Function")===type){
                    ...
                    if(type.isDeclaratorModule && !ctx.isVModule(type) && Utils.isInterface(type) && !ctx.isDeclaratorModuleDependency(type)){
          ...
          if(objectType){
               right = objectType;
          }else if(origin && !stack.right.hasLocalDefined() && Utils.isModule(origin)){
