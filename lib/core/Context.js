...
class Context extends Token{
    ...
    constructor(compiOrVModule, plugin, variables, graphs, assets, virtuals, glob, cache){
        super()
        this.#plugin = plugin;
    ...
    createModuleImportReferences(module, context=null, graph=null){
        ...
        module.getImportDeclarations().forEach( item=>{
            if(item.source.isLiteral){
                Common.parseImportDeclaration(this, item, context, graph)
            }
        });
    }
    ...
    getModuleImportSource(source, context, sourceId=null){
        ...
        if(isString && Common.isExternalDependency(this.options.dependences.externals, source, context)){
            return source;
        }
    ...
    genUniFileName(source, suffix=null){
        ...
        if(path.isAbsolute(source) || query){
            ...
            suffix = suffix || ext;
            return path.basename(file, ext)+'-'+createHash("sha256").update(source).digest("hex").substring(0, 8) + suffix;
        }
        return source;
    }
