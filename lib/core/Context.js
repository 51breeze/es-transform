...
class Context extends Token{
        ...
        if(assets && assets.size>0){
            assets.forEach( asset=>{
                if(asset.file){
                    ...
                    let external = Common.isExternalDependency(this.options.dependences.externals, source, context)
                    let specifiers = null;
                    if(!external){
                        if(asset.assign){
                            specifiers = [
    ...
    getAssetsImportSource(source, context){
        if(this.options.emitFile){
            if(source.includes('?')){
                ...
                source = path.join(path.dirname(file) , this.genUniFileName(source))
            }
    ...
    genUniFileName(source){
        ...
        if(path.isAbsolute(source) || query){
            ...
            return path.basename(file, ext)+'-'+createHash("sha256").update(source).digest("hex").substring(0, 8) + ext;
        }
        return source;
    }
