...
class Context extends Token{
    ...
    hasDeclareModule(module){
        if(Utils.isCompilation(this.target)){
            ...
            if(compi && compi.modules.has(module.getName())){
                return true;
            }
    ...
    getModuleReferenceName(module, context=null, stack=null){
        let name = null;
        if(isVModule(module)){
        ...
        if(Utils.isModule(context)){
            ...
            if(!name){
                name = context.getReferenceNameByModule(module, true);
                if(name){
                    return name;
                }
            }
        }else if(Utils.isCompilation(context)){
            name = context.getReferenceName(module, null, true);
            if(name)return name;
        }
        if(name && this.hasDeclareModule(module)){
            return name;
        }
        if(!name){
            name = module.getName('_');
        ...
        this.#dependencies.forEach((deps, moduleOrCompi)=>{
            ...
            deps.forEach(depModule =>{
                ...
                if(moduleOrCompi !== depModule && this.isNeedBuild(depModule)){
                    ...
                    if(Common.isGlobalsDependency(this.options.dependency.globals, depModule.getName(), moduleOrCompi)){
                        return;
                    }
