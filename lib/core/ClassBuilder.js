...
import {
    ...
    KIND_CONST,KIND_METHOD, KIND_CLASS, KIND_INTERFACE, MODIFIER_STATIC, MODIFIER_ABSTRACT,
    MODIFIER_FINAL, KIND_ENUM_PROPERTY, KIND_STRUCT, KIND_STRUCT_COLUMN, MODIFIER_OPTIONAL} from "../core/Constant";
import {createCommentsNode, createMainAnnotationNode,getMethodAnnotations,createModuleReferenceNode,createStaticReferenceNode, createUniqueHashId} from "./Common";
...
class ClassBuilder{
    ...
    create(ctx){
        ...
        const creator = this.createCreator(
            ...
            this.createClassDescriptor(ctx, module, methods, members, classComments)
        );
        ...
        if(this.stack.compilation.mainModule===module){
            ...
            if(exportNode){
                if(exportNode.type==="Identifier"){
                    ...
                    const refName = '__'+module.id+'_export';
    ...
    createImplements(ctx, module, stack=null){
        ...
        this.implements = module.implements.map( (impModule)=>{
            if(impModule.isInterface && !impModule.isStructTable && ctx.isActiveModule(impModule, module, true)){
    ...
    createDefaultInheritAccessor(ctx, methodStack, accessor){
        ...
        if(inheritModule && !inheritModule.isDeclaratorModule){
            ...
            const desc = inheritModule.getDescriptor(key, (desc)=>{
                if(Utils.isModifierPrivate(desc)){
                    return false;
                }
                if(accessor==='getter' && desc.isMethodGetterDefinition || accessor==='setter' && desc.isMethodSetterDefinition){
                    return true
                }
            })
            if(desc){
    ...
    createMemberDescriptor(ctx, node){
        ...
        if( node.isAccessor ){
            ...
            if(node.type==='PropertyDefinition'){
                if(node.init){
                    properties.push(createProperty('value', node.init));
                }
            }else{
        ...
        const confitNode = ctx.createObjectExpression(properties)
    ...
    createClassDescriptor(ctx, module, methods, members, classComments){
        const properties = [];
        let kind = module.isEnum ? KIND_CLASS : module.isStructTable ? KIND_STRUCT : module.isInterface ? KIND_INTERFACE :  KIND_CLASS;
