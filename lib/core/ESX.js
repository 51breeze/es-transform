...
function createChildren(ctx, children, data){
    ...
    while(true){
        ...
        if( last ){
            ...
            if( last.cmd.includes('each') || last.cmd.includes('for') ){
                hasComplex = true;
            }
            push(content, value);
        }
    ...
    if(content.some(item=>!!item.maybeIsArrayNodes)){
        let childNods = ctx.createArrayExpression(content);
        if( content.length > 1 ){
            childNods.newLine = content.some(item=>item.type ==="CallExpression");
        }
        return ctx.createCallExpression( 
            ctx.createMemberExpression([
                childNods,
    ...
    if(hasComplex){
        ...
        }else if(content.length > 1 && content.some(item=>item.isForNode)){
            let base =  content.shift();
            if(!base.isForNode){
