        ...
        if( last ){
            ...
            if( hasIf ){
                if( result && result.cmd.includes('elseif') ){
                    ...
                    const endNode = last.child.isSlot && !last.child.isSlotDeclared ? ctx.createLiteral(undefined) : createCommentVNode(ctx, 'end if', true)
    ...
    if(fills || all){
        ...
        if(all || fills.includes('for') || fills.includes('each')){
            if(!stack.isDirective && stack.directives && Array.isArray(stack.directives)){
                let directive = stack.directives.find(directive=>forNameds.includes(directive.name.value().toLowerCase()));
                if( directive ){
                    isForContext = true
