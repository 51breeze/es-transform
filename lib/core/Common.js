...
function createRouteInstance(options, module, owner, path, method, meta=null, params=[], defaultValue={}, isRouterModule=false, isWebComponent=false){
    let action = null;
    ...
    if(!hasFull){
        ...
        if(withNs && !hasFull && module.namespace){
...
function getModuleRedirectNode(ctx, module){
    ...
    if(redirectAnnot){
        ...
        if(pathArg){
            ...
            const value = String(path.value);
            const toModule = Namespace.globals.get(value);
            if(toModule){
                ...
                if(route){
                    ...
                    redirectPath = createRouteCompletePathNode(ctx, route, paramNode);
                }else{
                    redirectPath = ctx.createDefaultRoutePathNode(toModule);
                }
            }else{
