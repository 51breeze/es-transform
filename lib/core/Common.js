        ...
        importSource.specifiers.forEach(spec=>{
            if(spec.type==='default' || spec.type==='namespace'){
                ...
                if(spec.type==='default'){
                    ...
                    const isModule = Utils.isModule(module);
...
function getMethodOrPropertyAlias(ctx, stack, name=null){
    if(Cache.has(stack, 'Common.getMethodOrPropertyAlias')){
        return Cache.get(stack, 'Common.getMethodOrPropertyAlias')
    }
    ...
    Cache.set(stack, 'Common.getMethodOrPropertyAlias', resolevName);
    return resolevName;
}

function getMethodOrPropertyHook(ctx, stack){
    if(!stack)return null;
    if(Cache.has(stack, 'Common.getMethodOrPropertyHook')){
        return Cache.get(stack, 'Common.getMethodOrPropertyHook')
    }
    ...
    Cache.set(stack, 'Common.getMethodOrPropertyHook', invoke);
    return invoke
}
