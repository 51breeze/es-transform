...
const Class={
    ...
    getMethod(moduleClass, methodName, kind='method', isUndefined=false){
        let defaultValue = isUndefined ? undefined : null
        if(!moduleClass)return defaultValue;
        let has = false;
        while(moduleClass){
            let classDescriptor = Class.getClassDescriptor(moduleClass);
            if(classDescriptor){
                let members = classDescriptor.members;
                ...
                moduleClass = classDescriptor.inherit;
            }else{
                break
            }
        }
    ...
    getSuperMethod(moduleClass, methodName, kind='method', isUndefined=false){
        let defaultValue = isUndefined ? undefined : null
        if(!moduleClass)return defaultValue;
        let descriptor = Class.getClassDescriptor(moduleClass);
        if(descriptor && descriptor.inherit){
            return Class.getMethod(descriptor.inherit, methodName, kind, isUndefined)
        }
        if(typeof moduleClass === "function"){
            let obj = Reflect.getPrototypeOf(moduleClass.prototype)
            let desc = getDescriptor(obj, methodName) || getDescriptor(_proto, methodName);
